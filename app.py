...
# Initialize session state to hold the report data
if 'report_data' not in st.session_state:
    st.session_state.report_data = None

st.sidebar.header("Inputs")
startup_name_input = st.sidebar.text_input("Startup Name", placeholder="e.g., Figma, Stripe, Perplexity")
sector_input = st.sidebar.text_input("Target Sector", value="SaaS")
...
# If there is report data in the session state, display it
if st.session_state.report_data:
    ...
    # Adapt the rules feedback for the PDF generator
    pdf_rules_feedback = [item['text'] for item in rules_feedback]
    
    # --- PDF Download ---
    pdf = PDFReport()
    pdf_output = pdf.generate(startup_name, company_data, llm_analysis, pdf_rules_feedback)
    st.download_button(
